...
def test_analysis_uses_security_when_sample_is_sufficient(security_type, expected):
    result = evaluate_analysis_risk(security_type, False, "controlled", 20, 20_000, 5_000, 100, 20)
...
def test_short_or_empty_sample_is_unknown(duration, received, transmitted):
    result = evaluate_analysis_risk(
        "WPA3_SAE", False, "controlled", duration, 20_000, 5_000, received, transmitted,
    )
...
def test_full_capture_uses_aggregate_traffic_scope():
    result = evaluate_analysis_risk(
        "WPA3_SAE", False, "full", 20, 20_000, 5_000, 100, 20,
    )
...
def test_dominant_outbound_volume_raises_low_risk_for_review():
    result = evaluate_analysis_risk(
        "WPA3_SAE", False, "full", 20, 300_000, 1_200_000, 100, 100,
    )
...
def test_dominant_outbound_volume_does_not_reduce_high_network_risk():
    result = evaluate_analysis_risk(
        "OPEN", False, "full", 20, 300_000, 1_200_000, 100, 100,
    )
    assert result.level == "high"


def test_captive_portal_is_explained_without_being_called_malicious():
    result = evaluate_analysis_risk(
        "WPA3_SAE", True, "full", 20, 20_000, 5_000, 100, 20,
    )
